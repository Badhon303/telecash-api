...
const ExportAgentsButton = () => {
  const handleExport = async () => {
    try {
      const response = await fetch(`/api/agents?pagination=false`)
      const data = await response.json()

      if (!data?.docs || !Array.isArray(data.docs) || data.docs.length === 0) {
        alert('No agents found to export.')
        return
      }
      ...
      const filename = `Agents_Export_${datetimeString}.xlsx`
...
export default ExportAgentsButton
